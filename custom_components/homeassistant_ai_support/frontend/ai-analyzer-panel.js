class AIAnalyzerPanel extends HTMLElement {
  ...
  _getHistory() {
    if (!this._hass) {
      console.warn('AI Analyzer: hass object not available');
      return;
    }
    
    console.log('AI Analyzer: Calling websocket API to get history');
    this._hass.callWS({
      type: 'homeassistant_ai_support/get_history',
    }).then(result => {
      console.log('AI Analyzer: Got history result', result);
      this._history = result.history || [];
      this._render();
    }).catch(err => {
      console.error('AI Analyzer: Error fetching AI analyzer history:', err);
    });
  }

  async _runAnalysis() {
    try {
      console.log('AI Analyzer: Running new analysis');
      await this._hass.callService('homeassistant_ai_support', 'analyze_now');
      console.log('AI Analyzer: Analysis service called successfully');
      // Odczekaj trochę dłużej, aby analiza miała czas się zakończyć
      setTimeout(() => this._getHistory(), 5000);
    } catch (err) {
      console.error('AI Analyzer: Error running analysis:', err);
    }
  }

  _render() {
    if (!this._hass) {
      console.warn('AI Analyzer: Cannot render, hass object not available');
      return;
    }

    console.log('AI Analyzer: Rendering panel with', this._history.length, 'history items');
    ...
    // Dodaj sekcję debugowania (można usunąć w wersji produkcyjnej)
    if (this._hass && this._hass.development) {
      const debugInfo = document.createElement('div');
      debugInfo.className = 'debug-info';
      debugInfo.textContent = `Status połączenia: ${this._hass ? 'Połączono' : 'Brak połączenia'}\n`;
      debugInfo.textContent += `Liczba elementów historii: ${this._history ? this._history.length : 0}\n`;
